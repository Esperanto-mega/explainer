...
class PGExplainer(ExplainerAlgorithm):
    ...
    def __init__(self, epochs: int, lr: float = 0.003, **kwargs):
        ...
        self.node_linear = Linear(14, 64)
        self.node_convs = ModuleList()
        self.node_bns = ModuleList()

        for _ in range(2):
            conv = ARMAConv(in_channels = 64, out_channels = 64)
            self.node_convs.append(conv)
            self.node_bns.append(BatchNorm(64))

        self.edge_linear_1 = Linear(128, 64)
        self.edge_linear_2 = Linear(3, 64)
        ...
        params_dict = [
            {'params': self.node_linear.parameters(), 'lr': lr},
            {'params': self.node_convs.parameters(), 'lr': lr},
            {'params': self.edge_linear_1.parameters(), 'lr': lr},
            {'params': self.edge_linear_2.parameters(), 'lr': lr},
    ...
    def train(
        ...
        **kwargs,
        ...
        e1 = self.edge_linear_1(e)
        e2 = self.edge_linear_2(edge_attr)
        
        inputs = torch.cat([e1, e2], dim=1)
